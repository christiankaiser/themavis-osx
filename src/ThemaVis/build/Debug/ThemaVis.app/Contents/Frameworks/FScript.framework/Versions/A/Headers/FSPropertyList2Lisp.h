...
@interface FSPropertyList2Lisp : NSObject {
  ...
  NSInteger varNumber[FSPROPERTYLIST_MAXPLISTTYPE];
  NSString *varPrefix[FSPROPERTYLIST_MAXPLISTTYPE];
  NSUInteger objectCount[FSPROPERTYLIST_MAXPLISTTYPE];
  NSMutableSet *begunAddresses,*cycleAddresses;
  NSMutableDictionary *names;
  NSMutableString *letDefs;
  BOOL defineCycles;
  NSInteger needDictDef,needArrayDef;
...
- (void)addToMacroString:(NSMutableString *)lispString fromArrayOrString:(id)plist level:(NSInteger)level;
